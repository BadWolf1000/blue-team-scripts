...
#
# WHEN TO RUN:
#   - FIRST THING on every Linux box 
#   - Any time you think red team has stolen credentials
#   - After red team is detected on a machine
#
# LOG FILE: $HOME/blueteam_logs/password_changes.log
# ============================================================

LOG="$HOME/blueteam_logs/password_changes.log"
mkdir -p "$(dirname "$LOG")"
...
option_change_default() {
    echo ""
    echo -e "${BOLD}  Change all existing account passwords${NC}"
...
while true; do
    ...
    echo "  [4] Change all existing account passwords"
