...
#
#   Step 2 - When prompted, enter your team's new password.
#
#   Step 3 - Run it:
#            sudo bash harden_linux.sh
#
#   Step 4 - Review the output. Look for any [!] warnings.
#            The script logs everything to $HOME/blueteam_logs/harden_<timestamp>.log
#
...
# Prompt for new password before redirecting stdout to log
echo -n "[?] Enter new password for all accounts: "
read -rs NEW_PASS; echo ""
if [[ ${#NEW_PASS} -lt 8 ]]; then
    echo "[!] Password must be at least 8 characters. Exiting."; exit 1
fi
...
mkdir -p "$HOME/blueteam_logs"
LOGFILE="$HOME/blueteam_logs/harden_$(date +%Y%m%d_%H%M%S).log"
